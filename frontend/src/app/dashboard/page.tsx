...
function buildOrgFromGerencias(
  gerencias: { id: number; nombre: string; categoria?: string }[],
  defaultStructure: OrgCategory[]
): OrgCategory[] {
  const byCat: Record<string, string[]> = {};
  for (const g of gerencias) {
    const cat = g.categoria || "General";
    if (!byCat[cat]) byCat[cat] = [];
    byCat[cat].push(g.nombre);
  }
  return defaultStructure.map((d) => ({
    ...d,
    items: byCat[d.category] || [],
  }));
}
...
const DocumentManager: React.FC<{
  ...
  refreshDocs,
    ...
    const [ocultos, setOcultos] = useState<{ inbox: number[]; sent: number[] }>({ inbox: [], sent: [] });

    useEffect(() => {
      getMisOcultos().then((r) => setOcultos(r)).catch(() => {});
    }, [documents.length, docView]);
    ...
    const filteredDocs = documents.filter((doc) => {
      if (docView === "inbox" && ocultos.inbox.includes(doc.id)) return false;
      if (docView === "sent" && ocultos.sent.includes(doc.id)) return false;
      ...
      if (docView === "inbox") {
        ...
        if (doc.receptor_id && user?.id && String(doc.receptor_id) === String(user.id)) return true;
        if (canViewDept && doc.receptor_gerencia_id) {
          const myGerId = user?.gerencia_id?.toString();
          if (myGerId && myGerId === String(doc.receptor_gerencia_id)) return true;
        }
        const docDept = (doc.targetDepartment || "").toLowerCase().trim();
        const userDeptLower = (userDept || "").toLowerCase().trim();
        if (docDept && userDeptLower && docDept === userDeptLower) return true;
        return false;
      } else {
        if (canViewAll) return true;
        if (doc.remitente_id && user?.id && String(doc.remitente_id) === String(user.id)) return true;
        return false;
      }
    });
    ...
    const suggestedCorrelativo = useMemo(() => {
      const g = gerencias.find((gg: any) => String(gg.id) === String(user?.gerencia_id));
      const siglas = g?.siglas || (g?.nombre ? g.nombre.slice(0, 3).toUpperCase() : "COR");
      return `${siglas} ___ ${new Date().getFullYear()}`;
    }, [gerencias, user?.gerencia_id]);

    const handleUploadClick = () => {
      setSelectedFiles([]);
      setDocName("");
      setCorrelativo(suggestedCorrelativo);
      setMessageContent("");
      setTargetUserId("");
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (isDuplicate) {
        alert("Este archivo (mismo nombre y tamaño) ya está en la lista. No se permiten duplicados.");
        if (fileInputRef.current) fileInputRef.current.value = "";
        return;
      }
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 backdrop-blur-md p-4 animate-in fade-in duration-300">
            <div className={`w-full max-w-lg rounded-2xl border shadow-2xl overflow-hidden ${darkMode ? "bg-slate-900 border-slate-800" : "bg-white"}`}>
              ...
              <form onSubmit={confirmUpload} className="p-6 space-y-4">
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1 tracking-wider">
                    Correlativo (Siglas gerencia + ID + Año; editable)
                  </label>
                  <input
                    value={correlativo}
                    onChange={(e) => setCorrelativo(e.target.value)}
                    placeholder={suggestedCorrelativo}
                    className={`w-full px-4 py-2.5 rounded-lg border outline-none text-sm font-mono ${darkMode ? "bg-slate-950 border-slate-700 text-white" : "bg-slate-50 border-slate-200"}`}
                  />
                </div>
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-center">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <button
                          onClick={async () => {
                            if (!confirm("¿Quitar este mensaje de tu bandeja? Quedará registrado en tu historial.")) return;
                            try {
                              await ocultarDocumentoDeBandeja(doc.id, docView);
                              if (user?.id) createAuditLog({ user_id: user.id, evento: `Documento ${doc.id} (${doc.name}) ocultado de ${docView === "inbox" ? "entrada" : "enviados"}`, nivel: "info" }).catch(() => {});
                              getMisOcultos().then(setOcultos).catch(() => {});
                              refreshDocs();
                            } catch (e) {
                              console.error(e);
                              alert("Error al quitar de la bandeja.");
                            }
                          }}
                          className={`p-2 rounded-md transition-colors ${darkMode ? "hover:bg-slate-800 text-slate-400 hover:text-red-400" : "hover:bg-slate-100 text-slate-600 hover:text-red-600"}`}
                          title="Quitar de mi bandeja (queda en logs)"
                        >
                          <Trash2 size={16} />
                        </button>
                      </div>
...
export default function Dashboard() {
  ...
  const fetchDocuments = useCallback(async () => {
    try {
      ...
      const mappedDocs = data.map((d: any) => {
        ...
        const toFullUrl = (path: string) => (path && path.startsWith("http") ? path : `${API_BASE}${path?.startsWith("/") ? path : "/" + path}`);
        const rawFile = d.url_archivo || d.fileUrl || (d.archivos && d.archivos.length > 0 ? d.archivos[0] : null);
        const fileUrl = rawFile ? toFullUrl(rawFile) : undefined;
        const archivosFull = (d.archivos || []).map((url: string) => toFullUrl(url));
        if (fileUrl && !archivosFull.length) archivosFull.push(fileUrl);
  ...
  const fetchGerencias = useCallback(async () => {
    try {
      ...
      setGerencias(data);
      // Preservar módulos personalizados (ej. VI. Módulo de Desarrollo y Control Raíz) al actualizar desde API
      const defaultCats = DEFAULT_ORG_STRUCTURE.map((d) => d.category);
      setOrgStructure((prev) => {
        const fromApi = buildOrgFromGerencias(data || [], DEFAULT_ORG_STRUCTURE);
        const custom = prev.filter((g) => !defaultCats.includes(g.category));
        return [...fromApi, ...custom];
      });
    } catch (e) {
  ...
  const fetchTickets = useCallback(async () => {
    try {
      ...
      const mapped = data.map((t: any) => ({
        ...
        createdAt: new Date(t.fecha_creacion).toLocaleDateString("es-ES"),
        owner: t.solicitante_nombre || "Usuario",
  ...
  }, [userRole, user, gerencias.length]);
  ...
  // Cargar anuncio desde API o localStorage
  useEffect(() => {
    let loaded = false;
    getAnnouncement()
      .then((data) => {
        if (data && typeof data === "object" && (data.title || data.badge)) {
  ...
  // Handle ticket updates/creation in parent to sync with backend
  const handleTicketsChange = async (newTickets: Ticket[] | ((prev: Ticket[]) => Ticket[])) => {
    // This is a bit complex because TicketSystem calls setTickets directly.
    // We should ideally wrap this to persist to backend.
    // For now, let's just make sure they are fetched on mount.
    // In a full implementation, TicketSystem would call api.post/put.
    if (typeof newTickets === 'function') {
      setTickets(newTickets);
      // Note: This won't capture the new ticket to send to backend easily 
      // without modifying TicketSystem.tsx to be more 'prop-driven' for actions.
    } else {
      setTickets(newTickets);
    }
  };
    ...
    >
      ...
      `}
        ...
        <ChatWindow
          ...
          userRole={userRole}
          canConfigureLearning={hasPermission(PERMISSIONS_MASTER.CHATBOT_CONFIGURE_LEARNING)}
        />
