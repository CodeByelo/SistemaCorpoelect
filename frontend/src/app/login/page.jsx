...
const SplashScreen = ({ onComplete }) => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      // Actualizar estados - USAR FUNCIONAL PARA EVITAR BUCLE
      setStatus(prevStatus => {
        const newStatus = [...prevStatus];
        let changed = false;
        for (let i = 0; i < 3; i++) {
          const statusStart = 0.1 + (i * 0.2);
          if (progressValue > statusStart && !newStatus[i]) {
            newStatus[i] = true;
            changed = true;
          }
        }
...
const LoginCorpoelecForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // No borrar admin_scope_2026: es la configuración maestra del panel DEV y debe persistir entre logins
